...
class ImageController extends Controller
{
    ...
    public function indexAction($stylename, $filename)
    {
        ...
        if (file_exists($originalPath)) {
            ...
            $imageObject = $this->get('image.file_to_object')->getObjectFromFilename($filename, $imageEntityClass[0]);

            // Get crop coordinates if any.
            $crop = empty($imageObject) ? null : $imageObject->getCropCoordinates();

            $image = $this->get('image.imager')->createImage($originalPath, $stylePath, $style, $crop);
